...
fullLengthHMM::fullLengthHMM(
	...
	bool _tryToCompressedLiAndStephenSwitchSpace
) :
	...
	tryToCompressedLiAndStephenSwitchSpace(_tryToCompressedLiAndStephenSwitchSpace)
	
{
	compressedLiAndStephenSwitchSpace = false;
}
...
double fullLengthHMM::makeInference(std::string geneID, bool outputToFilestreams, std::ofstream& output_fasta, std::ofstream& output_fasta_masked, std::ofstream& output_graphLevels, std::string outputPrefix_furtherOutput, const std::set<std::string>& useReadIDs, std::map<std::string, double>& forRet_oneReadP_h1, std::map<std::pair<std::string, std::string>, double>& forRet_readPair_differentHaplotypes_P, std::map<unsigned int, std::map<std::pair<std::string, std::string>, double>>& forRet_genotypes_P, std::map<unsigned int, std::pair<std::map<std::string, double>, std::map<std::string, double>>>& forRet_allele_by_haplotype_P, std::vector<std::vector<std::string>>* forRet_samples_readAssignmentStates, std::vector<std::string>* forRet_readAssignmentStates_readIDs, size_t generateHaplotypeSamples, const std::map<std::string, double>* forConstraint_oneReadP_h1, const std::map<std::string, std::map<std::string, double>>* forConstraint_readPair_differentHaplotypes_P, const std::vector<std::vector<std::string>>* forConstraint_readAssignmentStates, const std::vector<std::string>* forConstraint_readAssignmentStates_readIDs)
{
	...
	compressedLiAndStephenSwitchSpace = (!constrainedReadAssignmentStates) && (tryToCompressedLiAndStephenSwitchSpace);
	//compressedLiAndStephenSwitchSpace = false;
	
	std::cout << "Gene " << currentGene << ", starting inference with " << readIDs.size() << " reads - compressedLiAndStephenSwitchSpace = " << compressedLiAndStephenSwitchSpace << ".\n" << std::flush;
	...
	*/
	...
	for(unsigned int first_level = 0; first_level < currentGene_geneLength; first_level++)
	{
		...
		if(statesByLevel.at(first_level).size() > max_states)
		{
			max_states = statesByLevel.at(first_level).size();
		}
	}
	std::cout << "Max. states: " << max_states << "\n";
	...
	for(unsigned int levelI = 0; levelI < currentGene_geneLength; levelI++)
	{
		...
		{
			std::vector<HMMtransition> transitions_to_next_level = computeLevelTransitions(levelI-1);
			n_jumps += transitions_to_next_level.size();
			assert(transitions_to_next_level.size());
		...
		double sum_forward_level_openMP = 0;
		#pragma omp parallel for reduction(+:sum_forward_level_openMP)    
		for(size_t stateI = 0; stateI < n_states ; stateI++)
		{
			sum_forward_level_openMP += statesByLevel.at(levelI).at(stateI).fw_p;			
		}			

		// std::cerr << "\t" << "C" << "\n" << std::flush;

		bool needToNormalize = false;
		if(sum_forward_level_openMP < 1e-20)
		{
			needToNormalize = true;
		}		
		
		if(((levelI % normalization_interval) == 0) || needToNormalize)
		{
	...
	std::cout << "Max. states: " << max_states << ", total states " << n_states_total << " (i.e. about " << (n_states_total/currentGene_geneLength) << " per level); transitions: " << n_jumps << "\n";
...
std::vector<HMMtransition> fullLengthHMM::computeLevelTransitions(size_t first_level, long long limitToFromState) const
{
	...
	if(compressedLiAndStephenSwitchSpace)
	{
		assert(first_level < stateSimplePositions.size());
		assert((first_level+1) < stateSimplePositions.size());
		if(stateSimplePositions.at(first_level) && stateSimplePositions.at(first_level+1))
		{
			...
			{
				forReturn.resize(statesByLevel.at(first_level).size());
				#pragma omp parallel for
				for(size_t stateI = 0; stateI < statesByLevel.at(first_level).size(); stateI++)
				{
					HMMtransition oneT;
