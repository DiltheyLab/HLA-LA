...
my $reads_removed_nonhomologous_mappings = 0;
my $n_reads = 0;
...
foreach my $readID (keys %readID_2_start_stop_MSAcolumns)
{
	...
	if(scalar(@genes) > 1)
	{
		...
		warn "Skipped read $readID with genes: " . join(', ', @genes);
	}
	else
	{
		my $geneID = $genes[0];
...
print "skipped_read_because_multiple_genes: $skipped_read_because_multiple_genes (pairs)\n";
print "reads_removed_nonhomologous_mappings: $reads_removed_nonhomologous_mappings\n";
		...
		print OUT_ACTIVEALLELES join("\t",
			...
			map {
				...
				if($in_MSA)
				{
					...
					die unless(all {defined $_} @haplotypes);
					$in_MSA_byHaplotype = join(',', map {die Dumper($_, $position, $allele, $remapping_alleles_present_at_position_byHaplotype{$position}{$allele}{$_}, $alleles_by_gene_forRemapping_count{$gene}{$_}) unless((defined $remapping_alleles_present_at_position_byHaplotype{$position}{$allele}{$_}) and (defined $alleles_by_gene_forRemapping_count{$gene}{$_})); 'preSelection_' . $_ . ':' . $remapping_alleles_present_at_position_byHaplotype{$position}{$allele}{$_} . '/' . $alleles_by_gene_forRemapping_count{$gene}{$_}} @haplotypes);
					$in_MSA_byHaplotypeSelected = join(',', map {
						# warn Dumper($gene, $_, $position, $allele, $remapping_alleles_present_at_position_bySelectedHaplotype{$position}{$allele}, $remapping_alleles_present_at_position_bySelectedHaplotype{$position}{$allele}{$_}) unless((defined $remapping_alleles_present_at_position_bySelectedHaplotype{$position}{$allele}{$_}) and (defined $selectedMSAReferenceAlleles_byHaplotype{$_}) and (defined $_));
						$remapping_alleles_present_at_position_bySelectedHaplotype{$position}{$allele}{$_} = 0 unless(defined $remapping_alleles_present_at_position_bySelectedHaplotype{$position}{$allele}{$_});
					$_ . ':' . $remapping_alleles_present_at_position_bySelectedHaplotype{$position}{$allele}{$_} . '/' . $selectedMSAReferenceAlleles_byHaplotype{$_}} @haplotypes);
...
foreach my $geneID (sort keys %gene_2_reads_after_trimming)
{
	...
	my $outerIteration = 0; 
	$use_reads_by_gene{$geneID}{$outerIteration} = {};
	$coverages_by_gene{$geneID}{$outerIteration} = [0, 0, 0, 0, 0];
...
foreach my $gene_ref_id (sort keys %translation_targets)
{
	...
	unless(exists $genotypes_by_MSAcolumn{$gene})
	{
		warn "No reads data (I) for gene $gene [$gene_ref_id]";
		next;
	}
	
	unless(exists $use_reads_by_gene{$gene})
	{
		warn "No reads data (II) for gene $gene [$gene_ref_id]";
		next;
	}
...
{
	die Dumper("Undefined $gene", \%use_reads_by_gene) unless(defined $gene);
	unless(defined $use_reads_by_gene{$gene})
	{ 
		warn Dumper("Undefined", $gene, $use_reads_by_gene{$gene});
	}
	unless(all {defined $_} keys %{$use_reads_by_gene{$gene}})
	{
		warn Dumper("Undefined keys", $gene, $use_reads_by_gene{$gene});		
	}
	
	unless(defined((max keys %{$use_reads_by_gene{$gene}})))
	{
		warn Dumper("Undefined whatever", $gene, $use_reads_by_gene{$gene});		
	}
...
sub processAlignments
{
	...
	die Dumper("Missing NM tag or weird read ID") unless(all {($_->[0][0] =~ /\/[12]$/) and ($_->[0][9] =~ /NM:i:/)} @$alignments_aref);
	
	my @alignments_r1 = grep {$_->[0][0] =~ /\/1$/} @$alignments_aref;
	my @alignments_r2 = grep {$_->[0][0] =~ /\/2$/} @$alignments_aref;
	die unless((scalar(@alignments_r1) + scalar(@alignments_r2)) == scalar(@$alignments_aref));
	
	my $max_alignmentLength_1 = max map {length($_->[1])} @alignments_r1;
	my $max_alignmentLength_2 = max map {length($_->[1])} @alignments_r2;

	my @alignments_r1_filtered_length = grep {length($_->[1]) == $max_alignmentLength_1} @alignments_r1;
	my @alignments_r2_filtered_length = grep {length($_->[1]) == $max_alignmentLength_2} @alignments_r2;
			
	my $min_NM_1 = min map {die unless ($_->[0][9] =~ /NM:i:(\d+)/); $1} @alignments_r1_filtered_length;
	my $min_NM_2 = min map {die unless ($_->[0][9] =~ /NM:i:(\d+)/); $1} @alignments_r2_filtered_length;

	my @alignments_r1_filtered_nm = grep {die unless ($_->[0][9] =~ /NM:i:(\d+)/); $1 == $min_NM_1} @alignments_r1_filtered_length;
	my @alignments_r2_filtered_nm = grep {die unless ($_->[0][9] =~ /NM:i:(\d+)/); $1 == $min_NM_2} @alignments_r2_filtered_length;
	
	# print "Filtered R1 from " . scalar(@alignments_r1) . " to " . scalar(@alignments_r1_filtered) . "; R2, from " .  scalar(@alignments_r2) . " to " . scalar(@alignments_r2_filtered) . "\n";
	
	
	my $convertAlignmentToArrays = sub {
		my $gene = shift;
		my $currentReferenceId = shift;
		my $currentReferenceFrom_1based = shift;
		
		my $readID_no12 = shift;

		my $aligned_reference = shift;
		my $aligned_read = shift;
		
		my $fillExternalHashes = shift;
		die unless(($fillExternalHashes == 0) or ($fillExternalHashes == 1));
		
		my $fullReadID_debug = shift;
		
		
		my $printDebug = $readID_no12 eq 'HLADPA1_h0_DPA1*01:03:01:04_3930_4336_2:0:0_2:0:0_94';
		if($printDebug)
		{
			print $fullReadID_debug, " -> ", $currentReferenceId, ":", $currentReferenceFrom_1based,  "\n", $aligned_reference, "\n", $aligned_read, "\n";
		}
		...
		for(my $i = 0; $i < length($aligned_read); $i++)
		{
			...
			print $c_ref_alignmentColumn, " " if($printDebug);;
		...
		return (\@MSA_columns, \@read_characters);
		
	};
		
	my $check_equal_columns = sub {
		my $alignments_aref = shift;
		my %MSA_column_alleles;
		my $readID_no12_outer;
		foreach my $read_alignment (@$alignments_aref)
		{
			(my $readID_no12 = $read_alignment->[0][0]) =~ s/\/[12]//;
			$readID_no12_outer = $readID_no12;
			...
			return 0 if($currentReferenceId eq 'pgf_Ns');
			return 0 unless(exists $alignments_translation_targets_href->{$currentReferenceId});
			...
			my ($MSA_columns_aref, $read_characters_aref) = $convertAlignmentToArrays->($gene, $currentReferenceId, $currentReferenceFrom_1based, $readID_no12, $aligned_reference, $aligned_read, 0);
			my @MSA_columns = @$MSA_columns_aref;
			my @read_characters = @$read_characters_aref;

			die unless(scalar(@MSA_columns) == scalar(@read_characters));
			for(my $i = 0; $i <= $#MSA_columns; $i++)
			{
				$MSA_column_alleles{join('/', $MSA_columns[$i], $read_characters[$i])}++;
			}
		}			
		if((defined $readID_no12_outer) and ($readID_no12_outer eq 'HLADPA1_h0_DPA1*01:03:01:04_3930_4336_2:0:0_2:0:0_94'))
		{
			print Dumper($alignments_aref->[0][0][0]);
		}
		return (all {$_ == scalar(@$alignments_aref)} values %MSA_column_alleles);
	};
	
	my $r1_equal_columns = $check_equal_columns->(\@alignments_r1_filtered_length);
	my $r2_equal_columns = $check_equal_columns->(\@alignments_r2_filtered_length);
	
	# print join("\t", $r1_equal_columns, $r2_equal_columns), "\n";
	
	unless($r1_equal_columns and $r2_equal_columns)
	{
		$reads_removed_nonhomologous_mappings++;
		return 0;
	}
	
	my @alignments_forProcessing;
	if(@alignments_r1_filtered_length)
	{
		push(@alignments_forProcessing, $alignments_r1_filtered_length[0]); 
	}
	if(@alignments_r2_filtered_length)
	{
		push(@alignments_forProcessing, $alignments_r2_filtered_length[0]); 
	}
	
	ALIGNMENT: foreach my $read_alignment (@alignments_forProcessing)
	{
		(my $readID_no12 = $read_alignment->[0][0]) =~ s/\/[12]//;
		...
		# if($readID_no12 eq 'HLADPA1_h0_DPA1*01:03:01:04_3930_4336_2:0:0_2:0:0_94')
		# {
			# print $read_alignment->[0][0], "\n", $aligned_reference, "\n", $aligned_read, "\n";
		# }
		...
		{		
			...
			my ($MSA_columns_aref, $read_characters_aref) = $convertAlignmentToArrays->($gene, $currentReferenceId, $currentReferenceFrom_1based, $readID_no12, $aligned_reference, $aligned_read, 1, $read_alignment->[0][0]);
			my @MSA_columns = @$MSA_columns_aref;
			my @read_characters = @$read_characters_aref;
