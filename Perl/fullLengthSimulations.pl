...
unless(-e $GRCh38_refGenome)
{
	$GRCh38_refGenome = '/gpfs/project/dilthey/references/GRCh38_full_analysis_set_plus_decoy_hla.fa';
	$GRCh38_refGenome_primary = '/gpfs/project/dilthey/references/Homo_sapiens.GRCh38.dna.primary_assembly.fa';
}
...
GetOptions (
	...
	'parameters_for_call2:s' => \$parameters_for_call2,
	'redo_stageII:s' => \$redo_stageII,
);
...
{
	my $prefix_truth = $outputPrefix . '/' . $individualPrefix;
	...
	my $fn_truth_types = $prefix_truth . '_HLATypes.txt';
	die "Missing truth types file: '$fn_truth_types'" unless(-e $fn_truth_types);	
	$fn_truth_types = abs_path($fn_truth_types);
	my $fn_issues_types = $fn_truth_types . $inference_sampleID_suffix . '.issues.txt';
	my $fn_classicalValidation_output = $fn_truth_types . $inference_sampleID_suffix . '.classicalValidation.txt';
	my $command_classical_validation = qq(cd .. && bash -c "./HLAtypeinference_validation.pl --sampleIDs all_${individualPrefix} --trueHLA $fn_truth_types --inference_sampleID_suffix $inference_sampleID_suffix --outputFn_issues $fn_issues_types &> $fn_classicalValidation_output");
	system($command_classical_validation) and die "Could not execute classical validation command: '$command_classical_validation'";
	...
	while(<SAMPLES>)
	{
		...
		push(@sampleIDs_and_BAMs, [$line_fields[0], $line_fields[1], $line_fields[2]]);
	}
	close(SAMPLES);
	
	my $validated_samples = 0;
	my %validationSummary;
	my %validationSummary_samples;
	for(my $sampleI = 0; $sampleI <= $#sampleIDs_and_BAMs; $sampleI++)
	{
		my $sampleID = $sampleIDs_and_BAMs[$sampleI][0];
		my $sampleID_inference = $sampleID . $inference_sampleID_suffix;
		my $BAM = $sampleIDs_and_BAMs[$sampleI][1];
		my $mutationRate = $sampleIDs_and_BAMs[$sampleI][2];
		
		$validationSummary_samples{$mutationRate}++;
		
		# next unless($mutationRate == 0);
		my $results_file = $working_dir . '/' . $sampleID_inference . '/hla/R1_bestguess.txt';
		unless(-e $BAM)
		{
			$BAM =~ s/^.+\/HLA-LA-devel\//${this_bin_dir}\/..\/..\//;	
		}
		die "BAM $BAM not existing" unless(-e $BAM);
		my $fn_done_stageII = $working_dir . '/' . $sampleID_inference . '/remap/haplotypeHMMInput.done.txt';;

		if(not -e $fn_done_stageII)
		{
			warn "'Done' file $fn_done_stageII not existing - skip.";
			next;
		}
		
		my $sampleID_workingDir = $working_dir . '/' . $sampleID_inference;
		...
		foreach my $locus (sort keys %inferred_haplotypes)
		{
			...
			my %graphLevels_with_mutation_details;
			foreach my $h (1, 2)
			{
				foreach my $graphLevel (sort keys %{$truth_haplotypes_byGraphLevel[$h-1]})
				{
					if(($graphLevel >= $minGraphLevel_bothHaplotypes) and ($graphLevel <= $maxGraphLevel_bothHaplotypes))
					{
						if($truth_haplotypes_byGraphLevel[$h-1]{$graphLevel} ne  $truth_haplotypes_byGraphLevel_preMutation[$h-1]{$graphLevel})
						{
							...
							# die if(defined $graphLevels_with_mutation_details{$graphLevel});
							$graphLevels_with_mutation_details{$graphLevel}{$h} = $truth_haplotypes_byGraphLevel[$h-1]{$graphLevel};
						}
					}
				}
			}
			...
			$validationSummary{$mutationRate}{$locus}{validatedAlleles} += (2 * ($maxGraphLevel_bothHaplotypes - $minGraphLevel_bothHaplotypes + 1));
			unless(defined $validationSummary{$mutationRate}{$locus}{alleleErrors_haplotype})
			{
				$validationSummary{$mutationRate}{$locus}{alleleErrors_haplotype} = 0;
				$validationSummary{$mutationRate}{$locus}{alleleErrors_genotype} = 0;
				$validationSummary{$mutationRate}{$locus}{mutationColumns} = 0;
				$validationSummary{$mutationRate}{$locus}{mutationColumns_correct_haplotype} = 0;
				$validationSummary{$mutationRate}{$locus}{mutationColumns_correct_genotype} = 0;
				$validationSummary{$mutationRate}{$locus}{mutationColumns_mutatedAllele_detected} = 0;
			}
			...
			foreach my $ordering ([1, 2], [2, 1])
			{
				...
				# this is not edit distance, but column differences
			...
			for(my $graphLevel = $minGraphLevel_bothHaplotypes; $graphLevel <= $maxGraphLevel_bothHaplotypes; $graphLevel++)
			{
				...
				# }
				my $minDifference;
				my $maxDetectedMutatedAlleles = 0;
				foreach my $ordering ([1, 2], [2, 1])
				{
					my $normalOrder = (($ordering->[0] == 1) and ($ordering->[1] == 2));
					...
					my $h1_carries_mutatedAllele = 0;
					my $h2_carries_mutatedAllele = 0;
					if(exists $graphLevels_with_mutation_details{$graphLevel})
					{
						if(exists $graphLevels_with_mutation_details{$graphLevel}{1})
						{
							if($normalOrder)
							{
								$h1_carries_mutatedAllele = 1;							
							}
							else
							{
								$h2_carries_mutatedAllele = 1;							
							}
						}
						if(exists $graphLevels_with_mutation_details{$graphLevel}{2})
						{
							if($normalOrder)
							{
								$h2_carries_mutatedAllele = 1;							
					...
					if($graphLevels_with_mutation_details{$graphLevel})
					{
						my $detectedAlleles = 0;
						if($h1_carries_mutatedAllele and (($diff_h1 == 0) or ($diff_h1_surrounding == 0)))
						{
							$detectedAlleles++;
						}						
						if($h2_carries_mutatedAllele and (($diff_h2 == 0) or ($diff_h2_surrounding == 0)))
						{
							$detectedAlleles++;
						}						
						# $detectedAllele = 1 if($detectedAllele > 1);
						if($maxDetectedMutatedAlleles < $detectedAlleles)
						{
							$maxDetectedMutatedAlleles = $detectedAlleles;
						}
					}
				}
			

				$validationSummary{$mutationRate}{$locus}{mutationColumns_mutatedAllele_detected} += $maxDetectedMutatedAlleles;
				...
				if($minDifference != 0)
				{
					...
					$validationSummary{$mutationRate}{$locus}{alleleErrors_genotype} += $minDifference;
				}
			}
			
			foreach my $mutatedGraphLevel (sort keys %graphLevels_with_mutation)
			{
				if(not exists $haplotype_errors_byGraphLevel{$mutatedGraphLevel})
				{
					$validationSummary{$mutationRate}{$locus}{mutationColumns_correct_haplotype}++;
				}
				if(not exists $genotype_errors_byGraphLevel{$mutatedGraphLevel})
				{
					$validationSummary{$mutationRate}{$locus}{mutationColumns_correct_genotype}++;
				}
			}			

			
			if($genotype_differences or $minEditDistance)
			{
				print "\t", $locus, "\n";
				...
				if($minEditDistance)
				{
					$validationSummary{$mutationRate}{$locus}{alleleErrors_haplotype} += $minEditDistance;	
				...
				foreach my $mutationColumn (sort keys %graphLevels_with_mutation)
				{
					$validationSummary{$mutationRate}{$locus}{mutationColumns}++;
				}
			}
			if(1 == 0)
			{
	...
	my @mutationRates_in_results = sort keys %validationSummary;
	foreach my $mutationRate (@mutationRates_in_results)
	{
		foreach my $locus (sort keys %{$validationSummary{$mutationRate}})
		{
			foreach my $field (sort keys %{$validationSummary{$mutationRate}{$locus}})
			{
				$validationSummary{'ALL'}{$locus}{$field} += $validationSummary{$mutationRate}{$locus}{$field};
			}
		}

	}
	foreach my $mutationRate (sort keys %validationSummary)
	{
		print "\n\nGrand summary for mutation rate $mutationRate - based on $validationSummary_samples{$mutationRate} validated samples\n";
		print "\t", join("\t", qw/Locus ValidatedAlleles GenotypeAccuracy HaplotypeAccuracy ErrorsGenotype ErrorsHaplotype MutatedColumns MutatedColumns_MutatedAlleleDetected MutatedColumns_CorrectGenotype MutatedColumns_CorrectHaplotype/), "\n";
		foreach my $locus (sort keys %{$validationSummary{$mutationRate}})
		{
			# print $locus, "\t", $validationSummary{$mutationRate}{$locus}{alleleErrors_haplotype}, "\n";
			
			my $haplotype_accuracy = sprintf("%.4f", ($validationSummary{$mutationRate}{$locus}{validatedAlleles} - $validationSummary{$mutationRate}{$locus}{alleleErrors_haplotype})/ $validationSummary{$mutationRate}{$locus}{validatedAlleles});
			my $genotype_accuracy = sprintf("%.4f", ($validationSummary{$mutationRate}{$locus}{validatedAlleles} - $validationSummary{$mutationRate}{$locus}{alleleErrors_genotype})/ $validationSummary{$mutationRate}{$locus}{validatedAlleles});
			my @output_fields = (
				$locus,
				$validationSummary{$mutationRate}{$locus}{validatedAlleles}, 
				$genotype_accuracy,
				$haplotype_accuracy,
				$validationSummary{$mutationRate}{$locus}{alleleErrors_genotype}, 
				$validationSummary{$mutationRate}{$locus}{alleleErrors_haplotype}, 
				$validationSummary{$mutationRate}{$locus}{mutationColumns},
				$validationSummary{$mutationRate}{$locus}{mutationColumns_mutatedAllele_detected},
				$validationSummary{$mutationRate}{$locus}{mutationColumns_correct_genotype},
				$validationSummary{$mutationRate}{$locus}{mutationColumns_correct_haplotype},			
			);
...
elsif($action eq 'applyAll')
{
	...
	for(my $sampleI = 0; $sampleI <= $#sampleIDs_and_BAMs; $sampleI++)
	{
		...
		if((not -e $fn_done_stageII) or ($redo_stageII))
		{
			my @commands;
			if(-e $results_file)
			{
			...
			{
				my $HLA_LA_cmd = qq(perl $HLA_LA_bin --BAM $BAM --graph PRG_MHC_GRCh38_withIMGT --sampleID $sampleID_inference --maxThreads 7);
				if($action2 eq 'do')
				{
					# print "Making inference for sample $sampleID ($sampleI / $#sampleIDs_and_BAMs)\n";
					print "\t", $HLA_LA_cmd, "\n" if($print_cmds);
					push(@commands, $HLA_LA_cmd);
				}
				else
				{
					print $HLA_LA_cmd, "\n" if($print_cmds);
			...
			if($parameters_for_call2)
			{
				$parameters_for_call2 =~ s/!/--/g;
			}
			my $HLA_LA_cmd_II = qq(perl $HLA_LA_bin --graph PRG_MHC_GRCh38_withIMGT --sampleID $sampleID_inference --maxThreads 7 --action call2 $parameters_for_call2);
			if($action2 eq 'do')
			{
				# print "Making inference (II) for sample $HLA_LA_cmd_II ($sampleI / $#sampleIDs_and_BAMs)\n";
				print "\t", $HLA_LA_cmd_II, "\n" if($print_cmds);
				push(@commands, $HLA_LA_cmd_II);
			}
			else
			{
				print $HLA_LA_cmd_II, "\n" if($print_cmds);
			...
			if(@commands)
			{
				...
				if($qsub)
				{
					my $qsub_fn = $working_dir . '/' . $sampleID_inference . '.qsub.fullLengthHaplotypeInference.bash';
					open(QSUB, '>', $qsub_fn) or die "Cannot open $qsub_fn";
					print QSUB qq(#!/bin/bash
#PBS -l select=1:ncpus=16:mem=64GB
#PBS -l walltime=48:00:00
#PBS -A 'IMMGEN'
#PBS -N fL_${sampleID_inference}

cd /gpfs/project/dilthey/projects/HLA-LA-devel/src
...
module load bcftools
					...
					close(QSUB);
					my $qsub_cmd = "qsub $qsub_fn";
					print $qsub_cmd, "\n";
					system($qsub_cmd) and die "Could not qsub: $qsub_cmd";
				}
				else
				{
					foreach my $cmd (@commands)
	...
	print "\n\nAction '$action' done -- $samples_complete already complete prior to running this, $samples_todo either still running (if qsub) or also done.\n\n";	
}
