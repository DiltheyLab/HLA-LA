...
my %ref_sequences;
{
	...
	$ref_sequences{'pgf'} = substr($referenceGenome_href->{$pgf_chr}, $pgf_start - 1, $pgf_stop - $pgf_start + 1);
...
my $pgf_reconstruction_aref = find_reconstruction(\@files_in_order, $ref_sequences{'pgf'});
